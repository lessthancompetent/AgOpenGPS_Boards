...
//   see run_tests.sh in this folder.  Flags: --no-observer --no-crab --no-ext
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
{
  if (argc < 2) { fprintf(stderr, "usage: %s log.csv [--no-observer] [--no-crab] [--no-ext]\n", argv[0]); return 2; }
  ...
  for (int i = 2; i < argc; i++)
  {
    ...
    if (!strcmp(argv[i], "--no-ext")) cfg.ext_speed = false;
  }
  coast_t c;
  coast_init(&c, &cfg);
