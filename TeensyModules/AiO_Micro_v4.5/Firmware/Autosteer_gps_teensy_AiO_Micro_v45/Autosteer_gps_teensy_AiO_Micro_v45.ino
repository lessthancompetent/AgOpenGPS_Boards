...
//
// connection plan (AiO Micro v4.5, from AiO-v4.5-Micro_Schematic_2024-03-27):
// Teensy Serial 7 RX (28) / TX (29) <- U18 "Single/Dual-Right" XBee socket, schematic nets RX1/TX1: Unicore UM982 (GGA + KSXT heading)
// Teensy Serial 2 RX (7) / TX (8)   <- U11 "Dual-Left" XBee socket, schematic nets RX2/TX2: SYD Dynamics TM171 AHRS (115200)
// Teensy Serial 3 RX (15)           <- RTK radio socket, net RX3
// Teensy Serial 5 RX (21)           <- net A7-RX5 (BNO085 RVC via JP3), unused; SerialGPS2 parked here (no second receiver)
// NOTE: the schematic net names RX1/TX1 are "GPS receiver 1", NOT Teensy Serial1. Teensy pins 0/1 (Serial1) and 20 (TX5) are no-connects.
//
// Configuration of receiver (UM982, on the UART wired to U18)
// Baud 460800 (must match baudGPS)
// Out: GGA 10 Hz, KSXT 10 Hz (KSXT carries the dual-antenna heading)
// In:  RTCM from AgIO / RTK radio
// Save to NVM. Twin-F9P RELPOSNED path stays compiled in but is unused.
...
HardwareSerialIMXRT* SerialGPS = &Serial7;   //Main position receiver (UM982 in U18, pins 28/29): GGA + KSXT
HardwareSerialIMXRT* SerialGPS2 = &Serial5;  //No second receiver on the UM982 build; parked on RX-only Serial5 so it never touches the TM171 on Serial2
...
void setup()
{
  ...
  // AiO Micro v4.5: the enclosure fixes the UM982 in U18 (Serial7, pins 28/29). Nothing to detect.
  SerialGPS = &Serial7;
  SerialGPS2 = &Serial5;
  Serial.println("GNSS on Serial7 (U18), SerialGPS2 parked on Serial5");
  ...
  if (!useCMPS)
  {
      for (int16_t i = 0; i < nrBNO08xAdresses; i++)
      {
        ...
        Serial.println("\r\nChecking for TM171 on Serial2 (U11)");

        if (TM171detectOnPort(&Serial2, 1500))
        {
            Serial.println("Received data from TM171 on Serial2");
            useTM171 = true;
        }
        else
        {
